...
#include <string.h>
#include <memory>
#include "pe_bliss/resource_cursor_icon_writer.h"
...
void resource_cursor_icon_writer::add_icon(const std::string& icon_file, const std::wstring& icon_group_name, uint32_t language, icon_place_mode mode, uint32_t codepage, uint32_t timestamp)
{
	...
	try
	{
		data_info = std::make_unique<resource_data_info>(res_.get_resource_data_by_name(language, pe_resource_viewer::resource_icon_group, icon_group_name));
